...
def pruebas_guardrails() -> bool:
    ...
    rechaza_invalido = orq.validar_argumentos(invalido["name"], invalido["input"]) is not None
    no_molesta = not orq.requiere_aprobacion(invalido)
...
def evaluar_caso(caso: dict) -> dict:
    ...
    if caso.get("falla_api"):
        ...
        orq.cliente = anthropic.Anthropic(base_url="http://localhost:9", max_retries=0, timeout=2.0)
    ...
    try:
        for mensaje in caso["mensajes"]:
            ...
            resultado = orq.hablar(thread_id, mensaje, lambda acciones: caso.get("aprobar", True))
    ...
    ok_ruta = traza[0] == caso["agente"] and traza[-1] == caso.get("termina_en", traza[-1])
    ok_efectos = len(orq.RECLAMOS) == caso["reclamos"]
