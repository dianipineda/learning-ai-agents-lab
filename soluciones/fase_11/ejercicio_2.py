...
def pedidos(estado: Estado) -> dict:
    escritor = get_stream_writer()  # canal para emitir eventos propios ("custom")
...
def correr(grafo, entrada, config, on_evento):
    ...
    pendiente = None
    for modo, chunk in grafo.stream(entrada, config, stream_mode=["updates", "custom"]):
        if modo == "custom":
...
def conversar(grafo, mensaje, decision, on_evento):
    ...
    if pendiente is not None:
        ...
        correr(grafo, Command(resume=decision), config, on_evento)
...
def estado_por_stream(grafo, mensaje):
    ...
    config = {"configurable": {"thread_id": str(uuid.uuid4())}}
    ultimo = None
    for chunk in grafo.stream({"mensaje": mensaje}, config, stream_mode="values"):
        ultimo = chunk
...
def main() -> int:
    ...
    chequear("en 'values' la traza SÍ está acumulada", ultimo["traza"] == ["clasificar", "pedidos"])
