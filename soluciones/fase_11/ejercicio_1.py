...
def es_inicio_de_turno(mensaje: dict) -> bool:
    ...
    if isinstance(mensaje["content"], str):
        return True
    return not any(bloque.get("type") == "tool_result" for bloque in mensaje["content"])
...
def recortar_historial(historial: list, max_mensajes: int) -> list:
    ...
    for i in inicios:  # del turno más antiguo al más reciente
        if len(historial) - i <= max_mensajes:
            return historial[i:]
    return historial[inicios[-1]:]  # ni el último turno cabe: se devuelve completo
...
def turno_simulado(historial: list, max_mensajes: int) -> list:
    ...
    enviados = recortar_historial(historial, max_mensajes)
...
def main() -> int:
    ...
    chequear(
        ...
        any(not validar_historial(HISTORIAL[-n:]) for n in range(1, len(HISTORIAL))),
    )
