...
def es_sospechoso(texto: str) -> bool:
    return any(re.search(patron, texto, re.IGNORECASE) for patron in PATRONES_INYECCION)


def neutralizar(texto: str) -> str:
    """Quita etiquetas que permitirían cerrar/abrir el sobre desde adentro del contenido."""
    return texto.replace("</resultado_tool>", "").replace("<resultado_tool>", "")


def envolver(texto: str) -> str:
    return f"<resultado_tool>{neutralizar(texto)[:MAX_CHARS]}</resultado_tool>"
