...
#
#     aprobado = interrupt(<lo que se le muestra al humano>)
#
...
def decidir(estado: Estado) -> str:
    ...
    ultimo = estado["historial"][-1]
    hay_sensible = any(
        b["type"] == "tool_use" and b["name"] in SENSIBLES for b in ultimo["content"]
    )
...
# -------------------------------------------------------------------
# Registra el nodo "aprobar" y ajusta las aristas:
#   - el mapa de add_conditional_edges necesita la nueva salida "aprobar"
#   - "aprobar" siempre sigue a "tools" (una arista fija)
#
...
builder.add_conditional_edges(
    "ejecutor", decidir, {"aprobar": "aprobar", "tools": "tools", END: END}
)
...
# 5b: retomar con el mismo config: grafo.invoke(Command(resume=<decisión bool>), config).
#     Se retoma en un while porque tras aprobar podría haber otra pausa.
